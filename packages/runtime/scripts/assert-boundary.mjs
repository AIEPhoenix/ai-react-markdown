...
if (!pkg.private) throw new Error('Legacy runtime must remain a private workspace package.');
const allowed = new Set(['@ai-react-markdown/engine', '@types/hast', '@types/mdast', 'unist-util-visit']);
...
for (const file of readdirSync('src', { recursive: true }).map(String)) {
  ...
  const check = (node) => {
    ...
    if (specifier && ts.isStringLiteral(specifier)) {
      ...
      } else if (!['@ai-react-markdown/engine', 'unist-util-visit', 'hast', 'mdast'].includes(name)) {
...
console.log('runtime boundary: private, framework-free source; environment gates folded');
