...
| `pipelineSession.ts`         | Merge coordinated parse options, append protected phantom suffixes, select full/incremental parsing, invalidate failed state, fall back to a full parse or, if that throws, to a plain-text frame | One session per rendered chunk                          |
...
An incremental failure clears the retained state before retrying the full pipeline. If the full pipeline itself throws (a hostile input such as thousands of nested raw `<div>` tags overflowing the HTML parser walk), the session logs the error in development builds and returns the frame as one plain-text paragraph whose positions cover the whole source, so one message cannot take the host surface down; the next frame parses normally. That last resort renders escaped text, not a stale frame, and it is not a substitute for an error boundary around adapter code. Each session and planner is mutable local computation state: do not share one across independent chunks or concurrently executing consumers.
