...
| `pipelineSession.ts`         | Merge coordinated parse options, append protected phantom suffixes, select full/incremental parsing, invalidate failed state, fall back to a full parse or, on the engine's raw-depth error only, to a plain-text frame | One session per rendered chunk                          |
...
An incremental failure clears the retained state before retrying the full pipeline. The full pipeline has one degraded outcome: when the engine's raw-HTML step reports `EngineRawHtmlDepthError` (element nesting deeper than the engine's bound, `RAW_HTML_MAX_DEPTH`, which hundreds of nested raw `<div>` tags exceed), the session logs the error in development builds and returns the frame as one plain-text paragraph whose positions cover the whole source, so one hostile message cannot take the host surface down; the next frame parses normally. That last resort renders escaped text, not a stale frame. Every other exception from the full pipeline propagates to the host, including a `RangeError` that is not that overflow and any throw from a remark or rehype plugin or handler the host supplied: those are bugs to surface, not frames to render as text, and the fallback is not a substitute for an error boundary around adapter code. Each session and planner is mutable local computation state: do not share one across independent chunks or concurrently executing consumers.
