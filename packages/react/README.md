...
Since 2.3.0 the Markdown engine lives in a separate package, [`@ai-markdown/engine`](https://www.npmjs.com/package/@ai-markdown/engine). It is an ordinary transitive dependency pinned to the React adapter's exact version, so **you never install, declare, or upgrade it yourself** — and it cannot drift out of step with core. The public API of this package did not change in the split.
...
| `enginePlugins`            | `readonly AIMarkdownEnginePlugin[]` | `defaultEnginePlugins` | Sealed engine-plugin selection — accepts core-exported plugin objects from `@ai-markdown/react/plugins` only. Absent → all five shipped plugins; passing an array replaces the set wholesale. See [Engine Plugins](#engine-plugins).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
