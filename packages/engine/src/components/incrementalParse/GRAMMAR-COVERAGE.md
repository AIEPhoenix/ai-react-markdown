...
where the construction does not hold. Two further gaps — duplicate node
signatures, and an append that only ADDS a node below the boundary — are
unreachable in this corpus and carry comments at the code rather than
defensive machinery.
