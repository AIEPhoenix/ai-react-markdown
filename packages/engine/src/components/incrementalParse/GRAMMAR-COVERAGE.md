...
| SCRIPT_DATA                 | `script`                                        | yes, plus the `p5Tok` script kind's `escaped` flag. A `<!--` inside `<script>` enters "escaped"; a nested `<script` then enters "double escaped", where `</script>` no longer ends the element while CommonMark ends the type-1 block at it. Irreconcilable, so it POISONS |
...
| Bogus comment still open at the blank (parse5)          | same shape: block ended, tokenizer did not                                                                                           | poison at the blank (the `p5Tok` bogus check in the blank branch)                                                                                                     |
