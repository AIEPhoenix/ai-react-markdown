...
Snapshots are not semantic proofs. Unit tests cover lifecycle and immutability contracts; real browsers cover Vue's three complete adapter paths. Packed consumers install tarballs outside the workspace and separately exercise ESM/CJS, dev/prod, SSR, and TypeScript. The existing release gate also requires fresh soak evidence for the candidate; an old beta's record cannot establish acceptance of a new candidate. Engine soak coverage must not be mistaken for core or adapter lifecycle coverage.
