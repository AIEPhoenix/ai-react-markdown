...
**Order caveat.** The registry orders chunks by _registration_ order, and both footnote numbering and the "last chunk" (which renders the aggregate footer) follow that order. A chunk that unmounts and later remounts — scrolled out of a virtualizer's window and back, or given a new React key — is released and re-registered at the **end** of the order: footnotes renumber document-wide and the aggregate footer moves to that chunk's tail, i.e. visually into the middle of the document. Only ever-mounted-once chunks keep a stable position. If you virtualize, keep the chunks that carry footnote **references** mounted (typically pin the last chunk), or accept that numbering/footer placement follows mount order rather than source order; a registry that orders by a caller-supplied stable index is on the roadmap.
