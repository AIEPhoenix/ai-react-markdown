...
本文件是 [包名、依赖与发布方案](./ai-markdown-packages-and-release.md) 的执行附件。包名、目录、共享 core 公共入口和外部依赖拆分已实施，五个新 scope 包已上传 npm 并通过实际产物消费验证；beta 标签清理及 GitHub 预发布仍在收尾。下表保留迁移前的事实与实施步骤，作为执行记录，不代表当前目录仍使用旧名称。文档站仍是后续工作。
