...
本文件是 [包名、依赖与发布方案](./ai-markdown-packages-and-release.md) 的执行附件。包名、目录、共享 core 公共入口和外部依赖拆分已实施，五个新 scope 包已上传 npm 并通过实际产物消费验证；GitHub 预发布已完成，维护者已明确接受四个 beta 包暂时保留 latest 标签。下表保留迁移前的事实与实施步骤，作为执行记录，不代表当前目录仍使用旧名称。文档站仍是后续工作。

## 当前实施进度

core/engine 公共入口已完成本轮收口，记录见 [API 审查](../api/core-engine-contracts.md)。Vue 已迁入 `packages/vue`，三条浏览器路径已有实现及自动化测试；旧原型仅保留归档指引。工作区版本为未发布的 `3.0.0-beta.2` 候选，仍须完成完整预检及发布门槛。以下旧目录表保留迁移前背景。
