...
- **P3:** `normalizeForMatch` strips only backslash + ASCII-punctuation escapes (a literal `\` in a label made the cross-chunk pre-check miss it); the currency-escape pass no longer splits the whole remainder into lines on every match; a later-mounted chunk repeating a footnote label renders the standalone mark while its own occurrence is still pending instead of blanking for a frame; mantine's `pre` override tolerates a string `className`; the mermaid "view SVG" window opens with `noopener`; the mermaid dark header icon uses a static dark token (explicit `colorScheme="dark"` under a light Provider left it invisible); the release workflow also checks that mantine's core peer range moved with the train; engine README (`katex` is an optional peer; the pipeline table lists exported symbols only), `SECURITY.md` (supported versions; `urlTransform={null}` is not an escape hatch — it means default), eslint config pointer.

Deferred to its own design pass: the incremental LaTeX wrapper retries a failing freeze candidate every frame (an early stray `$` or lone backtick makes the whole active region re-run three times per append — a perf inversion on such documents, not a correctness issue).
...
Verification: every fixed shape is a deterministic pin (red on 2.4.3); release soak (splice fuzz 50k / direction battery 20k / K=4 census, plus the LaTeX incremental 5000-stream and def-label 30k legs) clean on the final engine.
